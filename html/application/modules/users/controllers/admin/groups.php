...
class Groups extends Admin_Controller
{
    ...
    function edit()
    {
        ...
        if ($group_id)
        {
            ...
            $data['permissions'] = unserialize($group_permissions);
        }
        ...
        if ($this->form_validation->run() == TRUE)
        {
            ...
            if ($this->input->post('type') == 'administrator' && $this->input->post('permissions'))
            {
            	// Save any administrator permissions
                $Group->permissions = serialize($this->input->post('permissions'));
            }
            elseif ( ! $edit_mode)
            {
            	// Set new non-administrator groups permissions to null
            	$Group->permissions = NULL;
        	}
        	elseif ($edit_mode && $Group->modifiable_permissions)
        	{
        		// Set edited non-administrator groups permissions to null
        		$Group->permissions = NULL;
        	}
            elseif ($edit_mode && ( ! $Group->modifiable_permissions))
            {
            	// Note: Don't clear permissions if group had modifiable permissions disabled
            	$Group->permissions = $group_permissions;
            }
            elseif ($edit_mode && $Group->type == ADMINISTRATOR && $this->Group_session->type == SUPER_ADMIN)
            {
            	// Set edited administrator groups with no permissions checked to null
                $Group->permissions = NULL;
            }
