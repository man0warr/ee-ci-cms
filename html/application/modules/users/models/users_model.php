...
{	
     ...
     * Get User Profile
     *
     * Returns an array of user's profile fields and values.
     * Formats height to feet' inches"
     *
     * @return array
     */
    ...
    {
        $fields_array = array();
        $Profile_fields = $this->profile_fields->include_related('groups', 'title')->include_join_fields()->order_by('sort')->get();
        
        foreach($Profile_fields as $Profile_field)
        {
            if ($Profile_field->id == 9)
            {
                $height_feet = $Profile_field->join_value;;
            }
            elseif ($Profile_field->id == 10 && isset($height_feet))
            {
                $height_inches = $Profile_field->join_value;
                $fields_array['Height'] = $height_feet . '\' ' . $height_inches . '"';
            }
            else
            {
                $fields_array[$Profile_field->title] = nl2br($Profile_field->join_value);
            }
        }
     ...
     * Profile Picture
     *
     * Returns users profile picture if it exists or just a general image
     *
     * @return string
     */
    function profile_picture($large = FALSE)
    {
        if ( ! file_exists(FCPATH . USER_DATA . $this->id . '/profile_images/' . $this->id . '.jpg'))
        {
            return site_url('application/modules/users/assets/images/default_profile_picture.jpg');
        }
        elseif ($large)
        {
            $image_path = USER_DATA . $this->id . '/profile_images/';
            return site_url($image_path . $this->id . '.jpg') . '?' . time();
        }
        else
        {
            $image_path = USER_DATA . $this->id . '/profile_images/';
            return image_thumb($image_path . $this->id . '.jpg', 166, 300, FALSE, $image_path) . '?' . time();
        }
    }
     ...
     * @return bool
     */
    function profile_picture_exists()
    {
        if ( ! file_exists(FCPATH . USER_DATA . $this->id . '/profile_images/' . $this->id . '.jpg'))
        {
            return FALSE;
    ...
    function create_session($admin_id = null)
    {
        ...
        $Group_class->name = $this->groups->name;
