...
class Navigations_library
{
    ...
    protected function _set_config($config = array())
    {
        ...
        $this->start_nav_with_children_of_current = str_to_bool($this->start_nav_with_children_of_current);
    }
    ...
    public function initialize($config)
    {
        ...
        elseif ($this->start_nav_x_levels_above_current > 0)
        {
            $calculated_depth = $this->current_depth - $this->start_nav_x_levels_above_current;
        ...
        // Get a nav subset
        if (($this->start_nav_from_parent_depth > 0) || ($this->start_nav_x_levels_above_current > 0))
        {
        ...
        elseif ($this->start_nav_with_children_of_current)
        {
    ...
    protected function _list_nav($array, $depth = 1)
    {
        ...
        // Remove hidden items from array
        ...
        foreach($array as $Item)
        {
            ...
            if ( ! empty($this->_content))
            {
                $item_array = array(
					...
					'top' => ($Item->parent_id == 0) ? TRUE : FALSE,
                    'children' => (empty($Item->sub)) ? FALSE : TRUE
                );
